...
const OKRs = () => {
  ...
  const { lists, boards, statements, objectives, loading } = useSelector(
  ...
  const filterOKRsByDate = () =>
    dispatch(actionCreators.filterOkrs(dates.year));

  const _boards =
    boards &&
    boards.filter(
      (el) => el.creator === userId && el.boardType !== "Lean Canvas"
    );
  const current_board = _boards && _boards.slice(-1).pop();
  const current_boardID = current_board && current_board._id;
  const filteredStatements =
    statements && statements.filter((el) => el.boardId === current_boardID);
  const quarter1 =
    objectives &&
    objectives.filter(
      (el) =>
        el.boardId === current_boardID &&
        el.quarter === 1 &&
        el.archive === false
    );
  const quarter2 =
  ...
  const archived =
    objectives &&
    objectives.filter(
      (el) => el.boardId === current_boardID && el.archive === true
    );

  const todoLists =
    lists &&
    lists.filter(
      (el) => el.boardId === current_boardID && el.archive === false
    );
  ...
  const onDragEnd = (result) => {
    ...
    const newDestList = todoLists.find(
