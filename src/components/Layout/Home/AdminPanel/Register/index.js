...
const Register = ({ signup, state, setState, setCatalyzer }) => {
	...
	const selectValues = [
		...
		{
			id: '10',
			value: 'Economic Misfit',
			link: 'https://airtable.com/shrdriyhRdFT3nOjo',
			key: 'appXH1YhoI4ll5sxT'
		},
		...
		{
			id: '11',
			value: 'Devine Renewable Energy',
			link: 'https://airtable.com/shrEut5jM1Ukl4KO6',
			key: 'appoXAPWZMqkdEb8Y'
		},
	...
	const handleSignUp = () => {
		dispatch(
			actionCreators.signUp(state.username, state.email, state.password, state.base_key, state.link, (res) => {
				if (res.success === true) {
					setState({
	...
	return (
		<div className="catalyzer-card">
			<div className="register-form-nav">
				<h3>Catalyzer</h3>
				<CloseIcon
					className="register-form-container-icon"
					style={{ fontSize: '25px', color: 'rgba(0,0,0,0.3)' }}
					onClick={() => setCatalyzer(false)}
				/>
			</div>
			<div className="catalyzer-form">
				<div className="catalyzer-drop-down">
				...
				{visible ? (
					<div className="catalyzer-drop-down-modal">
						{selectValues.map((selector) => (
							<div
								...
								onClick={() => {
									setState({
										...state,
										username: selector.value,
										link: selector.link,
										base_key: selector.key
									});
									setVisible(false);
								}}
							>
